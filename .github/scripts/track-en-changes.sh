#!/usr/bin/env bash
#
# Track changes in php/doc-en and create issues in doc-ja
# for files that need translation updates.
#
# How it works:
#   - Fetches all commits on doc-en master from the last 7 days
#   - For each commit, checks if a matching issue already exists in doc-ja
#   - If not, creates one listing the JA files to update
#
...
while read -r SHA; do
  ...
  # Skip if no JA-relevant files
  if [ -z "$UPDATE_LIST" ] && [ -z "$NEW_LIST" ]; then
    echo "  -> No JA-relevant files, skipping."
    continue
  fi
